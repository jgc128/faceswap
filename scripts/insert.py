...
class Insert():  # pylint:disable=too-few-public-methods
    """ The Faceswap Face Insertion Process.
...
class Predict():
    ...
    arguments: :class:`argparse.Namespace`
        ...
        line arguments
    """
    def __init__(self, in_queue, queue_size, faces, arguments):
    ...
    def _get_io_sizes(self):
        ...
        first_key = list(self._faces.keys())[0]
        retval = dict(input=self._faces[first_key][1].shape[0], output=self._faces[first_key][1].shape[0])
        logger.debug(retval)
    ...
    def _compile_feed_faces(self, feed_faces, target_face):
        ...
        feed_faces = [target_face[2][0] for feed_face in feed_faces]
